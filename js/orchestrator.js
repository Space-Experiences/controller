...
function toggleClimateDisplay(command){
  ...
  if(command == true){
    TweenMax.to(climateDisplay,.15,{scale:1,ease:Power3.easeOut,opacity:1});
    console.log('show climate')
  }else{
    TweenMax.to(climateDisplay,.1,{scale:0,opacity:0});
...
if(climateCardShowing == false){
  ...
  TweenMax.to('.climate-card',.15,{y:"100%"});
...
function setTemperatureKnob(){
  ...
  TweenMax.set('#current-temp-knob',{rotation:calcRotationToDeg(Number(portalState.climate.f),true)});
  TweenMax.set('#control-knob',{rotation:calcRotationToDeg(Number(portalState.climate.setTemp),true)});
  ...
  channel.trigger('client-event', {eventType:'command',
                                      ...
                                      params:{deg:deg,wemos:false}});
                                        ...
                                        // Subscribe to listen for return event pub.
                                          ...
                                          // Subscribe to listen for return event pub.
                                          EventBus.subscribe('responseTemperatureSet', function(){
                                            resetTempControlKnob();
                                            TweenMax.killTweensOf(setTempElem);
                                            TweenMax.set(setTempElem,{clearProps:"all"});
                                          TweenMax.from(setTempElem,.8,{y:20,opacity:0,ease:Quad.easeOut});
